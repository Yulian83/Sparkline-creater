...
class Sparkline:
    ...
    @classmethod
    def verify_y_coord(cls, y_coord: list[int | float]):
        for coord in y_coord:
            if not (type(coord) is int or type(coord) is float):
                raise ValueError(f'the y_coord field only accepts float and int types')     
    ...
    @y_coord.setter
    def y_coord(self, y_coord: list[int | float]):
        self.verify_y_coord(y_coord)
...
sl = Sparkline(y_coord=[6, 7, 2, 2, 122, 1000, 12], title=True)
sl.y_coord = [1, 2, 3, 4, 1, 1]
result = sl.create()
print(result)
